...
//post the apply leave
exports.addApplyLeave = async(req, res) => {
        try {
            let data = await calculateHoliday4Leave(req.body.fromDate, req.body.toDate);
            ...
            req
                .checkBody(
                    "fromDate",
                    `From date of leave must be valid and after ${
          LP[`${req.body.leaveType}`]
        } day from today`
      )
      .isAfter(
        new Date(
          calculateNextDay(LP[`${req.body.leaveType}`] - 1)
        ).toDateString()
      );
    ...
    req
      .checkBody(
        "toDate",
        "End date of leave must be valid and after start date"
      )
      .isAfter(new Date(req.body.fromDate).toDateString());
    ...
    req.checkBody("userid", "Invalid Data").notEmpty();
    req.checkBody("userdate", "Invalid Data").notEmpty();
    req.checkBody("usergender", "Invalid Data").notEmpty();
    ...
    req.body.leaveDay <= 0
      ? errors.push({ param: "leaveDay", msg: "Leave must be positive Number" })
      : "";
    ...
    if (JSON.stringify(user) !== JSON.stringify(useri)) {
      errors.push({
        param: "Form Data",
        msg: "Invalid Data or Data not Found"
      });
    } else {
      ...
      //probation period
      if (monthDiff < 6) {
        if (leave_data.leaveType !== "unpaid")
          errors.push({
            param: "Leave Type ",
            msg: "Employee on Probation Period can only have Unpaid leave"
          });
      } else {
        //user gender validation
        if (user.gender == leave_data.usergender) {
          if (user.gender == "male" && leave_data.leaveType == "maternity") {
            errors.push({
              param: "leaveType",
              msg: "No Maternity Leave for this employee"
            });
          } else if (
            user.gender == "female" &&
            leave_data.leaveType == "paternity"
          ) {
            errors.push({
              param: "leaveType",
              msg: "No Paternity Leave for this employee"
            });
          } else if (user.gender == "other")
            errors.push({ param: "user", msg: "User Data Invalid" });
        } else {
    ...
    if (errors.length > 0) {
      ...
      usl = await calculateHoliday4Leave(
        leave_data.fromDate,
        leave_data.toDate,
        leaveType
      );
      uls = await calculateLeave(
        leave_data.fromDate,
        leave_data.toDate,
        usl,
        leaveType,
      ...
      if (uls.length != 2) {
        ...
        // res.send(leave_data);
        req.flash("success_msg", `Leave Apply have been added to pending list`);
...
//get apply leave edit form
exports.getApplyLeaveEdit = async (req, res) => {
  let id = req.params.id;
  var uid = req.query.uid;
  console.log(id + "-" + uid);
  var ldd = await getLeaveData();
  var data = await lSQL.getLeaveDataRecordByIdandUser(id, uid);
  var userdata = await uSQL.getUserById(uid);
  // var user = {
  //   'id':userdata.id,
  //   'name':userdata.name,
...
exports.UpdateApplyLeave = async (req, res) => {
  try {
    ...
    req.checkBody("leaveReason", "Reason should not be empty").notEmpty();
    req
      .checkBody("id", "Invalid Data")
      .notEmpty()
      .equals(req.params.id);
    req
      .checkBody("userid", "Invalid Data")
      .notEmpty()
      .equals(req.query.uid);
    req.checkBody("userdate", "Invalid Data").notEmpty();
    req.checkBody("usergender", "Invalid Data").notEmpty();
    ...
    /************************data valdiation************************** */
    console.log(errors);
    //res.render('applyLeave/editUserLeaveForm', { errors, ldd, data: req.body });
    if (errors.length > 0) {
      res.render("applyLeave/add", { errors, data: leave_data, ldd });
      return;
    } else {
      ...
      const leave_data = req.body;
      leave_data.employee_id = uid;
      let chkId = await lSQL.getLeaveRecordById(
        fid,
        uid,
      ...
      if (!chkId) {
        ...
        if (JSON.stringify(user) !== JSON.stringify(useri)) {
          ...
          if (monthDiff < 6) {
            ...
            if (user.gender == leave_data.usergender) {
              if (
                user.gender == "male" &&
                leave_data.leaveType == "maternity"
              ) {
                errors.push({
                  param: "leaveType",
                  msg: "No Maternity Leave for this employee"
        ...
        var usl,
          uls,
          flag = false; //to store no of day of casual, leave unpaid and holiday
        console.log(leave_data);
        //here goes code for user holidays validation
        usl = await calculateHoliday4Leave(
          leave_data.fromDate,
        ...
        if (uls.length != 2) {
          ...
          res.render("applyLeave/editUserLeaveForm", {
            errors,
            data: leave_data,
            ldd
          });
          ...
          if (
            form_status != "" &&
            form_status.affectedRows == 1 &&
            form_status.changedRows == 1
          ) {
...
//generate date array from start to end of leave
...
async function calculateHoliday4Leave(startDate, endDate, flag) {
  ...
  userLeaveStatus.forEach(leaveDay => {
    ...
    if (moment(leaveDay.date).day() == 0 || moment(leaveDay.date).day() == 6) {
      //&& !leaveDay.isHoliday
      leaveDay.isHoliday = true;
...
async function calculateLeave(startDate, endDate, userLeaveStatus, type, uid) {
  ...
  var leaveAbleDay =
    netCasualDay < ull.casual ? parseInt(ull.casual - netCasualDay) : 0;
