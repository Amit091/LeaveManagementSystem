...
exports.applyFloatLeave = async(req, res) => {
    try {
        ...
        if (result != true) {
            errors.push({
                param: "date",
                msg: "leave for float holidays must be applied prior 1 week ago"
            });
        }
        var numbers = /^[0-9]+$/;
...
exports.createHoliday = async(req, res) => {
    try {
        ...
        if (req.body.description == "") {
            errors.push({
                param: "Holiday  description",
                msg: "Empty holiday  description"
            });
        }
        ...
        if (holiday != "") {
            errors.push({
                param: "name",
                msg: "Holiday with this name already exists"
            });
        }
    ...
    try {
        ...
        if (holidayName != "") {
            if (req.body.name == holiday.name) {
                ...
                if (errors.length > 0) {
                    const holiday = await hSQL.getHolidayById(id);
                    res.render("holiday/editholidays", {
                        allresult,
                        errors,
                        holiday,
                        types
